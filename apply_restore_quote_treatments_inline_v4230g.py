...
CSS_PATCH = r'''
/* v42.3.0g quote treatments restored inline */
...
def main():
    ...
    # Rimuove eventuale card separata preventivo rimasta.
    ...
    # Rimuove eventuali vecchi wrapper inseriti male.
    s = re.sub(
    ...
    # Trova card Costi e margini.
    card_pos = s.find('<Card title="Costi e margini"')
    if card_pos == -1:
        raise RuntimeError('Card "Costi e margini" non trovata')

    # Inserisce prima della form-grid dei campi costo, dentro la card.
    form_pos = s.find('<div className="form-grid">', card_pos)
    if form_pos == -1:
        raise RuntimeError('form-grid dentro Costi e margini non trovata')
    ...
    # Togli gli stili vecchi che creavano blocchi strani.
    css = re.sub(
    ...
    if "v42.3.0g quote treatments restored inline" not in css:
        css += "\n\n" + CSS_PATCH.strip() + "\n"

    CSS.write_text(css, encoding="utf-8")

    print("Patch v42.3.0g applicata: trattamenti reinseriti dentro Costi e margini.")
